<template>
  ...
  >
    ...
    <q-separator v-if="comment_string"/>
    <q-card-section v-if="comment_string">
      <div v-html="comment_string"></div>
    </q-card-section>
...
export default {
  ...
  computed: {
    ...
    Features () {
      return this.store.geoJson_features
    },
    open_bridge_id: {
      get () {
        return this.store.open_bridge_id
      },
      set (value) {
        this.store.setOpen_bridge(value)
      }
    },
    open_bridge_obj () {
    ...
    comment_string () {
      if (this.open_bridge_obj.original_time[0].start === 0) return ''
      const firstStart = this.open_bridge_obj.original_time[0].start
      const firstEnd = this.open_bridge_obj.original_time[0].end

      let resultString =  this.t('first_string', { firstStart, firstEnd })
      if (this.open_bridge_obj.original_time.length === 2) {
        const secondStart = this.open_bridge_obj.original_time[1].start
        const secondEnd = this.open_bridge_obj.original_time[1].end

        resultString += '<br/>' + this.t('second_string', { secondStart, secondEnd })
      }
      return resultString
    }
  },
