export default {
  header_title: 'Карта разводных мостов',
  header_toolbar_tab_list: 'Список мостов',
  header_toolbar_tab_menu: 'Меню',

  temperature: 'Температура',
  open: "Сведён",
  close: "Разведён",
  mostotrest: "Подробнее о мосте",
  will_open_min: "Сведётся через {minutes} мин",
  will_open: "Скоро сведётся",
  will_close_min: "Разводится через {minutes} мин",
  will_close: "Скоро разводится",

  not_closed: "Мост сегодня не разводится",

  time_footer: "Точное время в Петербурге",
  language_string: 'Язык',

  other: 'Функции',
  dark_mode: "Тёмная тема",

  write: 'Написать нам сообщение',
  write_header: 'Мы открыты к диалогу',
  write_contact: 'Ваш контакт',
  write_letter: 'Письмо',
  cancel: 'Отмена',
  send: 'Отправить',

  timing: 'Тайминг',
  timing_description: 'Передвигая ползунок, вы увидите обстановку по мостам в установленное время',

  update_available: 'Есть обновления<br/> Нажмите "Ок", чтобы обновить приложение',
  camera: 'Камера',
  info: 'Информация',
  last_update: 'Последнее обновление',
  site_mostotrest: 'Сайт Мостотреста',
  list_hint: 'Нажмите сюда чтобы увидеть расписание всех мостов списком',
  load_error: 'Не удалось загрузить свежее расписание. Попробуйте позже.',
  first_string: 'Мост сегодня будет разведен с {firstStart} до {firstEnd}',
  second_string: 'И повторно с {secondStart} до {secondEnd}'
}
